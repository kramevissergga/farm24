(() => {
    ...
    document.addEventListener("click", (function(e) {
        if (e.target.closest("[data-open]")) {
            e.target.closest("[data-open-parent]").classList.toggle("_open");
            clearOpenClasses(e.target.closest("[data-open-parent]"));
        } else clearOpenClasses();
    }));
    function clearOpenClasses(exept) {
        const openEls = document.querySelectorAll("[data-open-parent]._open");
        openEls.forEach((openEl => {
            if (openEl && openEl != exept) openEl.classList.remove("_open");
        }));
    }
    const headerCategoryItems = document.querySelectorAll(".catalog-header__item");
    const headerSubmenuItems = document.querySelectorAll(".submenu-header__menu");
    const headerCatalogButton = document.querySelector(".header__catalog-button");
    const headerCatalogButtonWrapper = document.querySelector(".header__catalog-button-wrapper");
    headerCategoryItems.forEach(((item, index) => {
        item.addEventListener("mouseenter", (e => {
            clearActiveClasses();
            item.classList.add("_active");
            if (headerSubmenuItems[index] && !e.target.closest(".submenu-header")) headerSubmenuItems[index].classList.add("_active");
        }));
        item.addEventListener("touchend", (() => {
            clearActiveClasses();
            item.classList.add("_active");
            if (headerSubmenuItems[index]) headerSubmenuItems[index].classList.add("_active");
        }));
    }));
    headerCatalogButton.addEventListener("mouseleave", (() => {
        if (window.innerWidth >= 839.98) {
            clearActiveClasses();
            console.log("+");
            headerCatalogButton.classList.contains("_active") ? headerCatalogButton.classList.remove("_active") : null;
            bodyUnlock(0);
        }
    }));
    headerCatalogButtonWrapper.addEventListener("mouseenter", (() => {
        if (window.innerWidth >= 839.98) {
            headerCatalogButton.classList.add("_active");
            bodyLock(0);
            if (!document.querySelector(".catalog-header__item._active")) {
                headerCategoryItems[0].classList.add("_active");
                if (headerSubmenuItems[0]) headerSubmenuItems[0].classList.add("_active");
            }
        }
    }));
    document.addEventListener("touchend", (e => {
        if (!e.target.closest(".catalog-header") && !e.target.closest(".header__catalog-button")) if (window.innerWidth >= 839.98) {
            clearActiveClasses();
            headerCatalogButton.classList.remove("_active");
        }
    }));
    headerCatalogButtonWrapper.addEventListener("touchend", (() => {
        if (headerCatalogButton.classList.contains("_active")) {
            clearActiveClasses();
            headerCatalogButton.classList.remove("_active");
        } else {
            headerCatalogButton.classList.add("_active");
    ...
    document.querySelectorAll(".menu__title").forEach((item => {
        item.addEventListener("click", (() => {
            item.closest("._active")?.classList.remove("_active");
            clearActiveClasses();
        }));
    }));
    function clearActiveClasses() {
        headerCategoryItems.forEach((el => el.classList.remove("_active")));
        headerSubmenuItems.forEach((el => el.classList.remove("_active")));
    }
    const productBodies = document.querySelectorAll(".product-m__body");
    productBodies.forEach((function(productBody) {
        const productImage = productBody.querySelector(".product-m__image-link img");
        const radioColorGroups = productBody.querySelectorAll(".radio-color-m");
        radioColorGroups.forEach((group => {
            const items = group.querySelectorAll(".radio-color-m__item");
            items.forEach((item => {
                if (item.classList.contains(`_active`)) item.dataset.img ? productImage.src = item.dataset.img : null;
                item.addEventListener("click", (function() {
                    items.forEach((i => i.classList.remove("_active")));
                    this.classList.add("_active");
                    this.dataset.img ? productImage.src = this.dataset.img : null;
                }));
            }));
        }));
    }));
